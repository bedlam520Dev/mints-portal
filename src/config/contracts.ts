...
const parseEnvContracts = () => {
	...
	return raw
		.split(',')
		.map((entry, index) => {
			const [slug, address, chain, label, standardEntry] =
				entry.split(':');
			if (!address?.startsWith('0x')) return null;
			const chainId = Number(chain) || 8453;
			const normalizedStandard =
				standardEntry?.toLowerCase() === 'erc721'
					? 'erc721'
					: 'erc1155';
			return {
	...
	? [
			{
				...
				chainId:
					Number(process.env.NEXT_PUBLIC_DEFAULT_CHAIN_ID) || 8453,
...
export const getContractConfig = (slug?: string) => {
	...
	return (
		dropContracts.find((contract) => contract.slug === slug) ||
		dropContracts[0]
	);
};
